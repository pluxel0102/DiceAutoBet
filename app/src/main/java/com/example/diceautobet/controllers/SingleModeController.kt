...
class SingleModeController(
    ...
    private suspend fun placeBet() {
        try {
            ...
            // НОВАЯ ЛОГИКА: Если в прошлом раунде была ставка "Не дубль", 
            // сначала прокручиваем список влево (возврат к началу)
            if (gameState.wasNoDoubleBetPlaced) {
                Log.d(TAG, "⬅️ Предыдущая ставка была 'Не дубль' → возвращаем список к началу")
                scrollBetListToStart()
            }
            ...
            if (settings.enableNoDoubleBet && gameState.shouldPlaceNoDoubleBet()) {
                Log.d(TAG, "⚡ После 3 ничьих подряд → добавляем ставку 'Не выпадет дубль' (200,000)")
                placeNoDoubleBet()
            }
    ...
    /**
     * Размещение ставки "Не выпадет дубль" - НОВАЯ ЛОГИКА
     * Всегда ставит 200,000 через 4 нажатия на область прокрутки
     */
    private suspend fun placeNoDoubleBet() {
        try {
            Log.d(TAG, "⚡ placeNoDoubleBet() START: ставка 200,000 через 4 нажатия")
            FileLogger.i(TAG, "⚡ Размещение ставки 'Не выпадет дубль': 200,000 (4 нажатия)")
            ...
            // Проверяем область "4 нажатия"
            if (!areas.containsKey(SingleModeAreaType.FOUR_CLICKS_AREA)) {
                Log.e(TAG, "❌ Область '4 нажатия' не настроена!")
                onDebugMessage?.invoke("⚠️ Область '4 нажатия' не настроена")
                return
            }
            
            // Проверяем область "Не выпадет дубль"
            ...
            // 1. Нажимаем на область "4 нажатия" 4 раза для прокрутки к ставке 200,000
            Log.d(TAG, "� Прокручиваем список ставок (4 нажатия)...")
            repeat(4) { clickNumber ->
                if (gameState.isPaused) {
                    Log.d(TAG, "🛑 Прокрутка прервана на клике ${clickNumber + 1}/4")
                    return
                }
                
                clickArea(SingleModeAreaType.FOUR_CLICKS_AREA)
                Log.d(TAG, "  → Клик ${clickNumber + 1}/4 на область прокрутки")
                delay(CLICK_DELAY_MS)
            }
            
            Log.d(TAG, "✅ Прокрутка завершена, ставка 200,000 выбрана")
            
            // 2. Нажимаем на "Не выпадет дубль" один раз
            clickArea(SingleModeAreaType.NO_DOUBLE_BET)
            Log.d(TAG, "✅ Ставка 'Не выпадет дубль' размещена: 200,000")
            onDebugMessage?.invoke("⚡ Ставка 'Не выпадет дубль': 200,000 (4 нажатия прокрутки)")
            
            // Устанавливаем флаг, что ставка "Не дубль" была размещена
            gameState = gameState.copy(wasNoDoubleBetPlaced = true)
            Log.d(TAG, "🏁 Флаг wasNoDoubleBetPlaced установлен в true")
    ...
    /**
     * Возврат к начальной ставке после "Не дубль" (4 нажатия влево)
     */
    private suspend fun scrollBetListToStart() {
        try {
            Log.d(TAG, "⬅️ scrollBetListToStart() START: возврат к базовой ставке")
            FileLogger.i(TAG, "⬅️ Прокрутка списка влево: возврат к начальной ставке")
            
            // Проверяем область "4 нажатия влево"
            if (!areas.containsKey(SingleModeAreaType.FOUR_CLICKS_LEFT)) {
                Log.e(TAG, "❌ Область '4 нажатия влево' не настроена!")
                onDebugMessage?.invoke("⚠️ Область '4 нажатия влево' не настроена")
                return
            }
            
            delay(CLICK_DELAY_MS)
            
            // Нажимаем на область "4 нажатия влево" 4 раза для возврата к началу списка
            Log.d(TAG, "⬅️ Прокручиваем список влево (4 нажатия)...")
            repeat(4) { clickNumber ->
                if (gameState.isPaused) {
                    Log.d(TAG, "🛑 Прокрутка влево прервана на клике ${clickNumber + 1}/4")
                    return
                }
                
                clickArea(SingleModeAreaType.FOUR_CLICKS_LEFT)
                Log.d(TAG, "  ← Клик ${clickNumber + 1}/4 на область прокрутки влево")
                delay(CLICK_DELAY_MS)
            }
            
            Log.d(TAG, "✅ Прокрутка влево завершена, список вернулся к начальной ставке")
            
            // ВАЖНО: После возврата нужно нажать на цвет, чтобы активировать базовую ставку
            if (gameState.isPaused) {
                Log.d(TAG, "🛑 Пауза перед кликом на цвет")
                return
            }
            ...
            if (areas.containsKey(colorArea)) {
                Log.d(TAG, "🎨 Нажимаем на цвет ${gameState.currentColor.displayName} для активации базовой ставки")
                clickArea(colorArea)
                delay(CLICK_DELAY_MS)
            } else {
                Log.e(TAG, "❌ Область цвета $colorArea не настроена!")
            ...
            onDebugMessage?.invoke("⬅️ Возврат к базовой ставке (4← + цвет)")
            
            // Сбрасываем флаг после возврата
            gameState = gameState.copy(wasNoDoubleBetPlaced = false)
            Log.d(TAG, "🏁 Флаг wasNoDoubleBetPlaced сброшен в false")
            
        } catch (e: Exception) {
            Log.e(TAG, "❌ Ошибка прокрутки влево", e)
            FileLogger.e(TAG, "❌ Ошибка возврата к начальной ставке: ${e.message}")
        }
    }
